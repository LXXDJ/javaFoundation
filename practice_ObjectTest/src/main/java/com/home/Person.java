...
//        }
    ...
    /* 1. void 메소드 출력
    *  2. String으로 변경하여 출력
    *     ex. ㅇㅇ는 '앉아'를 배웠어요 or ㅇㅇ는 '앉아'를 몰라요.
    *  3. boolean으로 변경하여 출력
    *     ex. ㅇㅇ는 '앉아'를 배웠어요 or ㅇㅇ는 '앉아'를 몰라요. */

    public void orderToDog(){
        Scanner sc = new Scanner(System.in);
        System.out.print("어떤 재주를 시킬까요? : ");
        String say = sc.nextLine();

//        myDog.doSomeThingVoid(say);     // void
//        System.out.println(myDog.doSomeThingVoid(say)); // String
        if(myDog.doSomeThingVoid(say)){ // boolean
            System.out.println(myDog.getDogName()+"(이)는 '"+say+"'(을)를 배웠어요.");
        }else{
            System.out.println(myDog.getDogName()+"(이)는 '"+say+"'(을)를 몰라요.");
        }
    }
}
