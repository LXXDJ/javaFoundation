...
public class KindsOfVariable {  // 클래스 영역의 시작
    ...
    *  전역변수(클래스 전역에서 사용할 수 있는 변수) == 필드 == 멤버변수(클래스가 가지는 멤버라는 의미) */

    /* non-static field를 인스턴스 변수라고 한다. (인스턴스 생성 시점에 사용 가능한 변수라는 의미) */
    private int globalNum;

    /* static field를 정적필드(클래스변수)라고 한다. (클래스 영역에 생성되는 변수라는 의미)*/
    ...
    public void testMethod(int num){    // ***** 메소드 영역의 시작
        ...
        *  매개변수도 일종의 지역변수로 생각하면 된다.*/
        ...
        System.out.println(num);        // 매개변수는 호출 시 값이 넘어와서 변경되기 때문에 초기화 불필요

//        System.out.println(localNum); // 지역변수는 선언 외 다시 사용(호출)하기 위해서는 반드시 초기화가 되어야 한다.
